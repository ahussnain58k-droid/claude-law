---
name: hallucination-risk-checker
title: Hallucination Risk Checker
version: 4.1.0
author: Ali Hussnain
...
> **detect unsupported facts, invented citations, false certainty, unverifiable quotes, and missing source support.**
...
| `scan` | Scan claims. |
| `add-claim` | Add claim. |
| `mark-supported` | Mark support status. |
| `remove-unsupported` | Create safer version without unsupported claims. |
| `export-verification-table` | Export table. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Extract all factual, legal, numerical, date, quote, and citation claims.
2. Compare each claim to provided sources.
3. Mark supported/unsupported/conflicting/unverified/current-research-needed.
4. Remove or downgrade unsupported claims.
...
- Specific dates, laws, case names, and quotes require source support.
- A true general concept may still be unsupported in the supplied matter.
- Confidence language must match evidence.
...
claims:
  -
    id: C1
    claim: [claim]
    type: fact|law|date|number|quote|citation
    support: supported|unsupported|conflicting|unverified
    source: [source]
```
...
```markdown
# Matter: [Client/Matter] — Hallucination Risk Checker
...
```text
Use `hallucination-risk-checker` with command `scan` on these documents. Store results in YAML and include section references.
```

```text
Use `hallucination-risk-checker` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `hallucination-risk-checker` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

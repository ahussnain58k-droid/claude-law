...
> **remove a skill safely while preserving user-created data, audit records, dependencies, and rollback instructions.**
...
|---|---|
| `plan-uninstall` | Plan uninstall. |
| `list-files` | List files. |
| `preserve-data` | Identify data to preserve. |
| `remove` | Create removal steps. |
| `export-uninstall-record` | Export record. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. List installed files and dependencies.
2. Separate skill files from user-created outputs.
3. Preserve audit records.
4. Define removal and verification steps.
5. Document reinstall conditions.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Deleting generated user data may be harmful.
- Shared dependencies can break other skills.
- Audit records should remain.
...
uninstall:
  skill: [skill]
  decision: planned|complete|blocked
files:
  -
    path: [path]
    action: remove|preserve|backup
```
...
```markdown
# Matter: [Client/Matter] — Skill Uninstaller
...
```text
Use `skill-uninstaller` with command `plan-uninstall` on these documents. Store results in YAML and include section references.
```

```text
Use `skill-uninstaller` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `skill-uninstaller` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

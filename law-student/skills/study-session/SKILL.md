...
> **run one focused study session with objective, active recall, practice, feedback, and next actions.**
...
| `ask` | Ask active-recall question. |
| `practice` | Give practice prompt. |
| `grade` | Grade answer. |
| `export-review` | Export session notes. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Set session objective.
2. Briefly review core concept.
3. Ask active-recall questions.
4. Give practice prompt.
5. Provide feedback and next review items.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Session should be interactive when user participates.
- Feedback should target reasoning, not just correctness.
- Use course materials when supplied.
...
```markdown
# Matter: [Client/Matter] — Study Session
...
```text
Use `study-session` with command `start-session` on these documents. Store results in YAML and include section references.
```

```text
Use `study-session` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `study-session` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

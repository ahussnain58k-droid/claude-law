...
> **triage a privacy-rights request and create a response plan with identity verification, deadline, exemptions, systems, owners, and audit record.**
...
|---|---|
| `intake` | Capture request type, requester, date, channel, jurisdiction, and verification status. |
| `list-requests` | List active DSAR records. |
| `edit-status` | Update status: received, verifying, searching, reviewing, responding, closed. |
| `calculate-deadline` | Calculate response deadline when jurisdiction and receipt date are supplied. |
| `route-search` | Assign system owners and search tasks. |
| `export-response-plan` | Export response plan and audit log. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Classify request type: access, deletion, correction, portability, restriction, objection, opt-out, appeal, or mixed.
2. Verify identity and agent authority before disclosing or deleting data.
3. Calculate deadline only when jurisdiction and received date are supplied; otherwise flag as unverified.
4. Map systems, vendors, backups, archives, and responsible owners.
5. Route exemption issues to legal review: privilege, litigation hold, security, fraud, third-party data, retention law, employee data, minor data, sensitive data.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Deletion may be limited by retention law, legal claims, fraud prevention, security, contracts, tax, or litigation hold.
- Access responses may need redaction of third-party data and privileged material.
- Authorized agents require authority verification before action.
- Jurisdiction changes deadline, scope, and appeal rights.
...
- privacy-impact-assessment if request reveals program gap
...
dsar:
  request_id: DSAR-001
  type: access|deletion|correction|portability|restriction|objection|opt_out|appeal|mixed
  received_date: YYYY-MM-DD
  jurisdiction: [jurisdiction]
  status: [status]
verification:
  identity_verified: yes|no|pending
  agent_authority: yes|no|not_applicable
system_searches:
  -
    system: [system]
    owner: [owner]
...
```markdown
# Matter: [Client/Matter] — DSAR Response
...
```text
Use `dsar-response` with command `intake` on these documents. Store results in YAML and include section references.
```

```text
Use `dsar-response` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `dsar-response` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

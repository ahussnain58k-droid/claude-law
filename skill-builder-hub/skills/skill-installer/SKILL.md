...
> **decide whether a skill package is safe to install by reviewing source, license, files, permissions, dependencies, and behavior.**
...
| `sandbox-test` | Create test plan. |
| `approve-install` | Approve/hold/reject install. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Inspect all files, not just SKILL.md.
2. Check license and attribution.
3. Check hidden instructions, scripts, hooks, connectors, network/file access, credentials, external actions.
4. Run sandbox test.
5. Return approve/hold/reject with rollback plan.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Skills are supply-chain dependencies.
- A safe-looking SKILL.md can be unsafe if scripts/hooks/connectors are unsafe.
- License mismatch can block public use.
...
```markdown
# Matter: [Client/Matter] — Skill Installer
...
```text
Use `skill-installer` with command `inspect` on these documents. Store results in YAML and include section references.
```

```text
Use `skill-installer` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `skill-installer` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

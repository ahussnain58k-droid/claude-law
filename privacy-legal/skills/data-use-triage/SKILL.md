---
name: data-use-triage
title: Data Use Triage
version: 4.1.0
author: Ali Hussnain
...
> **decide whether a proposed data use can proceed, needs controls, requires impact assessment, or must stop pending review.**
...
|---|---|
| `triage` | Classify data use and risk. |
| `add-use` | Add data use record. |
| `edit-purpose` | Change purpose or compatibility analysis. |
| `route-assessment` | Route to PIA/DPIA, DPA, AI risk, or legal review. |
| `export-decision` | Export triage decision. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify data source, original collection purpose, proposed purpose, data categories, subjects, sharing, retention, AI/model use, and jurisdictions.
2. Check compatibility with notices, consent, contract, policy, laws, and user expectations.
3. Classify decision: proceed, proceed with controls, PIA required, DPA required, AI assessment required, legal escalation required, or stop.
4. List required controls and owner before proceeding.
5. Flag if user asks to rely on consent/legitimate interest/contract without supplied basis.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Secondary use risk depends on user expectations and original notice.
- Combining datasets can create new sensitivity even if each dataset was low risk alone.
- AI training, enrichment, profiling, and sharing can materially change risk.
- Retention and deletion must be rechecked when purpose changes.
...
data_use:
  ...
  decision: proceed|controls|PIA_required|DPA_required|AI_assessment|required_legal_review|stop
controls:
  -
    control: [control]
    owner: [owner]
...
routes:
  -
    reason: [reason]
    route_to: privacy-impact-assessment|dpa-review|ai-risk-assessment|human-review-gate
```
...
```markdown
# Matter: [Client/Matter] — Data Use Triage
...
```text
Use `data-use-triage` with command `triage` on these documents. Store results in YAML and include section references.
```

```text
Use `data-use-triage` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `data-use-triage` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

...
> **convert one case into a student-ready brief with procedural posture, facts, issue, rule, holding, reasoning, dissent/concurrence, and cold-call questions.**
...
|---|---|
| `brief` | Create case brief. |
| `list-elements` | List issue/rule/holding/reasoning. |
| `add-note` | Add professor/class note. |
| `edit-brief` | Revise section. |
| `export-class-notes` | Export class-ready notes. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Use only supplied case text or citation excerpts.
2. Separate holding from dicta.
3. Identify procedural posture before rule.
4. Add facts that matter to reasoning only.
5. Create cold-call questions with short answers.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Do not invent case holdings.
- Dicta is not holding.
- Procedural posture changes meaning.
...
```markdown
# Matter: [Client/Matter] — Case Brief
...
```text
Use `case-brief` with command `brief` on these documents. Store results in YAML and include section references.
```

```text
Use `case-brief` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `case-brief` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

...
> **review a DPA for controller/processor roles, processing instructions, subprocessors, transfers, security, breach notice, audits, deletion, and assistance.**
...
|---|---|
| `review` | Run DPA review. |
| `set-role` | Set party roles: controller, processor, joint controller, business/service provider, operator/processor, or unknown. |
| `list-processing` | List data categories, data subjects, purposes, duration, subprocessors, and transfer locations. |
| `check-transfers` | Check international transfer terms and required safeguards. |
| `route-impact` | Route high-risk processing to privacy-impact-assessment. |
| `export-dpa-issues` | Export DPA issue table. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify privacy roles per processing activity, not only per company.
2. Map processing details: data categories, subjects, purpose, duration, subprocessors, transfers, retention, deletion, and security controls.
3. Review instructions, confidentiality, security, breach notice, subprocessor approval/notice, audits, assistance, deletion/return, and regulatory cooperation.
4. Route to privacy-impact-assessment if processing includes sensitive data, children, automated decision-making, profiling, new vendor, AI, large-scale monitoring, or cross-border transfer risk.
5. Flag conflicts between DPA, MSA, product behavior, privacy policy, and security exhibit.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- A party can be processor for one processing activity and controller for another.
- Subprocessor terms need notice, objection, flow-down, and responsibility mechanics.
- Breach notice needs timing, content, cooperation, and escalation path.
- Deletion/return obligations must align with backups, retention law, and litigation holds.
...
dpa_issues:
  -
    ...
    route_to: privacy-impact-assessment|privacy-gap-analysis|escalation-flagger
subprocessors:
  -
    name: [name]
    location: [country]
    notice: yes|no|unknown
    objection_right: yes|no|unknown
```
...
```markdown
# Matter: [Client/Matter] — DPA Review
...
```text
Use `dpa-review` with command `review` on these documents. Store results in YAML and include section references.
```

```text
Use `dpa-review` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `dpa-review` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

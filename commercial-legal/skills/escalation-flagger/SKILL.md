...
> **determine whether a matter needs legal, privacy, security, finance, executive, outside-counsel, or business-owner escalation.**
...
|---|---|
| `assess` | Assess escalation need. |
| `list-triggers` | List detected escalation triggers. |
| `add-trigger` | Add manual trigger from reviewer. |
| `assign-owner` | Assign escalation owner and deadline. |
| `draft-escalation` | Draft safe escalation summary. |
| `export-routing` | Export routing table. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Classify issue domains: legal, privacy, security, AI, finance, tax, employment, IP, litigation, regulatory, reputation, executive.
2. Rate severity and urgency separately.
3. Identify the decision that cannot safely be made by the current user.
4. Draft a minimal escalation summary that excludes unnecessary privileged/confidential details.
5. Route to specific skill if the escalation is actually an assessment need.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- High deal value is not the only escalation trigger; data, AI, public output, and waiver risk can require escalation.
- Business owner approval is not the same as legal approval.
- Outside counsel escalation may be appropriate when jurisdiction, litigation, privilege, or regulated advice is unclear.
...
escalations:
  -
    issue: [issue]
    domain: legal|privacy|security|AI|finance|executive|outside_counsel
...
```markdown
# Matter: [Client/Matter] — Escalation Flagger
...
```text
Use `escalation-flagger` with command `assess` on these documents. Store results in YAML and include section references.
```

```text
Use `escalation-flagger` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `escalation-flagger` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

---
name: skills-quality-auditor
title: Skills Quality Auditor
version: 4.1.0
author: Ali Hussnain
...
> **audit a SKILL.md for legal specificity, executable commands, security, YAML storage, governance, public trust, license, and maintenance quality.**
...
| `mark-risk` | Mark risk. |
| `export-audit` | Export audit report. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Check single-job specificity.
2. Check explicit command menu.
3. Check YAML storage schema.
4. Check prompt-injection and security rules.
5. Check obligation routing and human-review gates.
6. Check licensing/attribution.
7. Score approve/hold/reject.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- A specific skill beats a broad template.
- Executability requires command verbs and storage schema.
- Security must be visible in the file, not assumed from platform.
...
```markdown
# Matter: [Client/Matter] — Skills Quality Auditor
...
```text
Use `skills-quality-auditor` with command `audit` on these documents. Store results in YAML and include section references.
```

```text
Use `skills-quality-auditor` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `skills-quality-auditor` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

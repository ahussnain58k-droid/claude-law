...
> **configure a legal skill pack by storing user role, jurisdiction, review owners, privilege rules, distribution rules, and risk thresholds.**
...
| `edit-jurisdiction` | Set jurisdiction. |
| `edit-reviewer` | Set reviewer/approval owner. |
| `set-distribution` | Set privilege/confidentiality distribution rules. |
| `export-profile` | Export PROFILE.md/YAML. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Ask only setup-critical questions.
2. Capture jurisdiction, role, reviewer, audience, external-use rules, and sensitive-data rules.
3. Store profile in YAML.
4. Do not collect secrets or unnecessary personal data.
5. Route high-risk defaults to human-review-gate.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- A legal workflow without jurisdiction and reviewer is incomplete.
- Distribution rules should be set before real matter materials are pasted.
- Profile should not store privileged facts unless necessary.
...
```markdown
# Matter: [Client/Matter] — Cold Start Interview
...
```text
Use `cold-start-interview` with command `create-profile` on these documents. Store results in YAML and include section references.
```

```text
Use `cold-start-interview` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `cold-start-interview` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

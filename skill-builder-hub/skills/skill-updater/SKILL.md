...
> **review a skill update for behavior changes, permission changes, license changes, regression risk, and rollback requirements.**
...
|---|---|
| `compare` | Compare old and new. |
| `mark-change` | Mark behavior/security change. |
| `run-tests` | Define tests. |
| `approve-update` | Approve/hold/reject update. |
| `export-update-note` | Export note. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Compare old/new files.
2. Flag behavior, permission, dependency, license, security, and review-gate changes.
3. Run tests.
4. Create rollback plan.
5. Return approve/hold/reject.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Security language removal is a regression.
- New external action capability changes risk.
- License changes can block redistribution.
...
```markdown
# Matter: [Client/Matter] — Skill Updater
...
```text
Use `skill-updater` with command `compare` on these documents. Store results in YAML and include section references.
```

```text
Use `skill-updater` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `skill-updater` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

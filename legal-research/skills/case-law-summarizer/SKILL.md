...
> **summarize case law with holding, dicta, reasoning, procedural posture, factual limits, and use in argument.**
...
| `add-fact-limit` | Add factual limit. |
| `mark-dicta` | Mark dicta. |
| `edit-holding` | Revise holding. |
| `export-case-note` | Export note. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Use supplied case text.
2. Extract procedural posture and material facts.
3. Separate issue, holding, reasoning, dicta, and disposition.
4. Add factual limits and distinguishability.
5. Flag if treatment unknown.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- A case summary without posture can mislead.
- Holding must be tied to facts and issue.
- Later treatment matters.
...
```markdown
# Matter: [Client/Matter] — Case Law Summarizer
...
```text
Use `case-law-summarizer` with command `summarize` on these documents. Store results in YAML and include section references.
```

```text
Use `case-law-summarizer` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `case-law-summarizer` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

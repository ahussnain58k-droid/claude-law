...
> **produce a legal research plan with issue breakdown, authority hierarchy, search strings, databases, and verification tasks.**
...
| `add-issue` | Add research issue. |
| `add-search` | Add search string. |
| `rank-source` | Rank source type. |
| `export-research-log` | Export log. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Rewrite question into legal issues.
2. Identify jurisdiction and authority hierarchy.
3. Create search strings with synonyms/connectors.
4. Prioritize binding primary authority.
5. Create research log and verification tasks.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Secondary sources can orient but rarely settle binding law.
- Jurisdiction controls source hierarchy.
...
```markdown
# Matter: [Client/Matter] — Source Finder
...
```text
Use `source-finder` with command `plan` on these documents. Store results in YAML and include section references.
```

```text
Use `source-finder` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `source-finder` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

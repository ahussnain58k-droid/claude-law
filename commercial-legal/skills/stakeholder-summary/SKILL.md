...
> **convert legal findings into a plain-English summary for a specific stakeholder audience with decisions, risks, and next actions.**
...
|---|---|
| `summarize` | Create stakeholder-ready summary. |
| `set-audience` | Set audience: executive, sales, product, privacy, security, finance, student, client, outside counsel. |
| `list-decisions` | List decisions needed from stakeholders. |
| `edit-tone` | Adjust tone and detail level. |
| `redact` | Remove privileged/confidential details. |
| `export-email` | Export email-ready version. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify stakeholder audience and decision needed.
2. Remove legal analysis that should stay inside privilege circle unless user confirms distribution.
3. Translate every legal issue into business consequence, decision, owner, and deadline.
4. Keep source references for reviewer, but avoid overloading business audience.
5. End with decision options and next-step routing.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Business summaries should not waive privilege or over-distribute legal advice.
- Stakeholders need choices, not legal lectures.
- Risk language should be clear enough for non-lawyers to act.
...
- public-output-risk-review if external/public
- privilege-risk-checker if legal advice is summarized
...
summary:
  audience: [audience]
  purpose: [purpose]
  distribution: internal_privileged|business|external
decisions:
  -
    decision: [decision]
    owner: [owner]
...
redactions:
  -
    item: [removed detail]
    reason: privilege|confidentiality|personal_data
```
...
```markdown
# Matter: [Client/Matter] — Stakeholder Summary
...
```text
Use `stakeholder-summary` with command `summarize` on these documents. Store results in YAML and include section references.
```

```text
Use `stakeholder-summary` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `stakeholder-summary` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

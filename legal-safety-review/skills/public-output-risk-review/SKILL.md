---
name: public-output-risk-review
title: Public Output Risk Review
version: 4.1.0
author: Ali Hussnain
...
> **review public-facing legal content for unauthorized-advice risk, confidentiality, privilege, solicitation, jurisdiction, claims, and disclaimer needs.**
...
|---|---|
| `review` | Review public output. |
| `mark-risk` | Mark risk. |
| `rewrite-safe` | Rewrite safer version. |
| `add-disclaimer` | Add disclaimer. |
| `export-public-version` | Export version. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Classify content as education, marketing, commentary, legal information, or individualized advice.
2. Scan for client/matter facts, confidential data, privilege, guarantees, testimonials, outcome claims, jurisdiction overclaims, and solicitation.
3. Cite source for legal claims or mark unverified.
4. Rewrite into educational/plain-English language.
5. Return Publish, Revise, Legal Review Required, or Do Not Publish.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Public legal education can become legal advice when applied to a specific person's facts.
- Jurisdiction limits must be clear.
- Disclaimers help but do not cure all risks.
...
public_output:
  platform: [platform]
  audience: [audience]
  decision: publish|revise|legal_review_required|do_not_publish
risks:
  -
    text: [text]
    risk: [risk]
    action: keep|revise|remove|disclaimer
```
...
```markdown
# Matter: [Client/Matter] — Public Output Risk Review
...
```text
Use `public-output-risk-review` with command `review` on these documents. Store results in YAML and include section references.
```

```text
Use `public-output-risk-review` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `public-output-risk-review` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

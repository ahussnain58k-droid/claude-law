---
name: confidentiality-risk-checker
title: Confidentiality Risk Checker
version: 4.1.0
author: Ali Hussnain
...
> **identify and redact confidential, personal, trade-secret, client, matter, or sensitive information before sharing or publication.**
...
|---|---|
| `scan` | Scan for confidentiality risk. |
| `add-sensitive-item` | Add sensitive item. |
| `mark-redaction` | Mark redaction. |
| `create-safe-version` | Create redacted version. |
| `export-redaction-log` | Export log. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify audience and distribution.
2. Classify sensitive items.
3. Decide keep/redact/generalize/remove.
4. Produce safe version.
5. Create redaction log.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Confidentiality and privilege are related but distinct.
- Public examples should not contain client-identifying facts.
- Aggregation may still identify a party in niche contexts.
...
sensitive_items:
  -
    item: [text]
    type: client|matter|personal|trade_secret|financial|security|other
    action: keep|redact|generalize|remove
    reason: [reason]
```
...
```markdown
# Matter: [Client/Matter] — Confidentiality Risk Checker
...
```text
Use `confidentiality-risk-checker` with command `scan` on these documents. Store results in YAML and include section references.
```

```text
Use `confidentiality-risk-checker` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `confidentiality-risk-checker` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

...
> **manage installed skills as an inventory with owner, version, source, risk tier, permissions, review date, conflicts, and status.**
...
| `add-skill` | Add skill. |
| `edit-skill` | Edit record. |
| `disable-skill` | Mark disabled. |
...
3. Track permissions and dependencies.
4. Identify stale/conflicting skills.
5. Set review date and status.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- High-risk skills need owners and review cadence.
- Conflicting instructions can create unsafe behavior.
- Disabling is safer than deleting when audit trail matters.
...
```markdown
# Matter: [Client/Matter] — Skill Manager
...
```text
Use `skill-manager` with command `list-skills` on these documents. Store results in YAML and include section references.
```

```text
Use `skill-manager` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `skill-manager` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

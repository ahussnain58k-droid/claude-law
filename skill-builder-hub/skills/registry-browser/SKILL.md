...
> **evaluate candidate skills from a registry using source trust, license, security, maintenance, and workflow fit.**
...
|---|---|
| `search` | Search/evaluate candidates. |
| `rank` | Rank candidates. |
| `check-license` | Check license. |
| `check-trust` | Check maintainer/source trust. |
...
1. Clarify use case.
2. Filter candidates by source, license, maintenance, docs, permissions, and security posture.
3. Rank candidates.
4. Do not recommend install until installer checks pass.
5. Export shortlist.
...
- License and permissions matter as much as features.
- A skill without security policy should be treated as higher risk.
...
candidates:
  -
    ...
    recommendation: consider|avoid|review_required
```
...
```markdown
# Matter: [Client/Matter] — Registry Browser
...
```text
Use `registry-browser` with command `search` on these documents. Store results in YAML and include section references.
```

```text
Use `registry-browser` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `registry-browser` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

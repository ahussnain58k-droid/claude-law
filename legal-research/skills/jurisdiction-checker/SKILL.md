...
> **identify jurisdiction-dependent claims and rewrite overbroad legal statements into safer, source-limited wording.**
...
|---|---|
| `check` | Check jurisdiction risk. |
| `add-claim` | Add legal claim. |
| `set-jurisdiction` | Set jurisdiction/forum/governing law. |
| `rewrite-claim` | Rewrite safer claim. |
| `export-risk-table` | Export table. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Extract every legal claim.
2. Identify jurisdiction, forum, governing law, regulator, and date dependency.
3. Mark claim as supported, overbroad, unknown, or requires local counsel.
4. Rewrite universal claims narrowly.
5. Route research needs.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Contract governing law may differ from forum.
- Federal and state rules may both matter.
- Foreign law claims need source and date.
...
claims:
  -
    ...
    status: supported|overbroad|unknown
...
```markdown
# Matter: [Client/Matter] — Jurisdiction Checker
...
```text
Use `jurisdiction-checker` with command `check` on these documents. Store results in YAML and include section references.
```

```text
Use `jurisdiction-checker` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `jurisdiction-checker` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

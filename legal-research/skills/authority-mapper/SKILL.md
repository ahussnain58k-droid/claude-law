...
> **map authorities to issues by hierarchy, jurisdiction, relevance, treatment, factual fit, and confidence.**
...
|---|---|
| `map` | Create authority map. |
| `add-authority` | Add authority. |
| `edit-treatment` | Update treatment. |
| `rank-authority` | Rank strength. |
| `export-map` | Export table. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Classify authority as binding, persuasive, secondary, or background.
2. Map to issue/rule element.
3. Check court, date, jurisdiction, treatment, and factual fit.
4. Flag conflicts/splits.
5. Rank best authorities.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Binding status depends on forum.
- Dicta is weaker than holding.
- Negative treatment can destroy reliance.
...
authorities:
  -
    ...
    status: binding|persuasive|background|unknown
...
```markdown
# Matter: [Client/Matter] — Authority Mapper
...
```text
Use `authority-mapper` with command `map` on these documents. Store results in YAML and include section references.
```

```text
Use `authority-mapper` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `authority-mapper` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

...
> **build a law-course outline with rule hierarchy, elements, exceptions, cases, policy notes, and exam triggers.**
...
|---|---|
| `build` | Create outline. |
| `add-rule` | Add rule. |
| `add-case` | Add case. |
| `edit-topic` | Revise topic. |
| `export-attack-outline` | Export condensed exam outline. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Organize by topic/subtopic.
2. Add rule elements and exceptions.
3. Map cases to rules, not just names.
4. Add exam triggers and common traps.
5. Create condensed attack outline.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Case list without rule extraction is not useful.
- Exam outline should be shorter than study outline.
- Professor emphasis controls priority.
...
```markdown
# Matter: [Client/Matter] — Outline Builder
...
```text
Use `outline-builder` with command `build` on these documents. Store results in YAML and include section references.
```

```text
Use `outline-builder` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `outline-builder` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

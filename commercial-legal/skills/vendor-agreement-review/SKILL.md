...
> **review vendor terms for service, security, data, renewal, liability, audit, transition, and operational dependency risk.**
...
|---|---|
| `review` | Run vendor agreement review. |
| `tier-vendor` | Classify vendor as low, standard, critical, regulated, or high-dependency. |
| `list-obligations` | List vendor and customer obligations with owners. |
| `route-impact` | Route data/security/AI/renewal obligations to the right assessment. |
| `edit-tier` | Change vendor tier after business-owner input. |
| `export-procurement-summary` | Export business-friendly summary for procurement. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Classify vendor criticality before issue rating.
2. Review services, implementation, acceptance, SLAs, support, fees, price increases, taxes, expenses, renewals, termination, transition assistance, and audit rights.
3. Review security, privacy, confidentiality, subprocessors, incident notice, compliance, insurance, sanctions, anti-bribery, and business continuity.
4. For every operational obligation, capture owner, trigger, deadline, evidence, and route.
5. Escalate if vendor is critical and lacks termination assistance, audit rights, data protections, or incident obligations.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- A low-value vendor can be high risk if it handles sensitive data or critical infrastructure.
- SLA credits may not be a meaningful remedy for mission-critical services.
- Vendor unilateral change rights can bypass negotiated protections.
- Audit rights matter more for regulated, security-sensitive, or critical vendors.
...
vendor:
  name: [Vendor]
  tier: low|standard|critical|regulated|high_dependency
...
risks:
  -
    ...
    action: accept|negotiate|escalate
```
...
```markdown
# Matter: [Client/Matter] — Vendor Agreement Review
...
```text
Use `vendor-agreement-review` with command `review` on these documents. Store results in YAML and include section references.
```

```text
Use `vendor-agreement-review` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `vendor-agreement-review` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

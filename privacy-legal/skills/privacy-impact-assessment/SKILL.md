...
> **create a privacy impact assessment for a product, process, vendor, dataset, AI use case, or data-sharing activity.**
...
| `list-assessments` | List assessment records and status. |
| `add-processing` | Add processing activity. |
| `edit-risk` | Edit inherent or residual risk. |
| `route-controls` | Assign mitigation owners. |
| `export-approval` | Export approval memo. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Define processing purpose, data source, data categories, data subjects, recipients, retention, transfers, vendors, and system owners.
2. Assess necessity, proportionality, transparency, lawful basis/justification, data minimization, retention, security, rights impact, and user expectations.
3. Identify high-risk triggers: sensitive data, children, employees, biometrics, location, health, financial, automated decision-making, profiling, AI, surveillance, large-scale monitoring, cross-border transfer.
4. Route to DPA review for processors/vendors, AI risk assessment for AI use, DSAR process review for rights impacts, and privacy gap analysis for program gaps.
5. Record residual risk and approval owner before marking approved.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Impact assessment should be per processing activity, not merely per product.
- A low-risk product can contain high-risk data processing.
- Substantial changes in purpose, data, vendor, model, or user population may require reassessment.
- Mitigations should be testable, assigned, and evidence-backed.
...
routes:
  -
    trigger: [trigger]
    route_to: dpa-review|ai-risk-assessment|privacy-gap-analysis
```
...
```markdown
# Matter: [Client/Matter] — Privacy Impact Assessment
...
```text
Use `privacy-impact-assessment` with command `create` on these documents. Store results in YAML and include section references.
```

```text
Use `privacy-impact-assessment` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `privacy-impact-assessment` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

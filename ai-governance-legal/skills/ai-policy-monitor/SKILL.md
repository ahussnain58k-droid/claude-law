...
> **detect drift between AI policy, AI inventory, approvals, actual AI use, substantial modifications, incidents, and training records.**
...
|---|---|
| `scan-policy` | Extract AI policy controls. |
| `compare-inventory` | Compare policy controls to AI inventory. |
| `add-drift` | Add drift finding. |
| `mark-substantial-modification` | Mark AI system change requiring reassessment. |
| `assign-remediation` | Assign remediation owner. |
| `export-drift-report` | Export AI policy drift report. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Convert policy into testable controls.
2. Compare each control to inventory, approvals, vendor records, risk assessments, incidents, training, and actual use reports.
3. Flag unapproved systems, unknown role/tier, missing owner, high-risk use without controls, and substantial modification without reassessment.
4. Route inventory issues to ai-inventory, high-risk use to ai-risk-assessment, vendor issues to vendor-ai-review, privacy issues to PIA/DPIA.
5. Create remediation backlog with owner, due date, evidence needed, and status.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Policy drift occurs when approved text exists but practice lacks evidence.
- A tool can be approved for one use case but noncompliant for another.
- Substantial modification should trigger reassessment even if the vendor/tool name is unchanged.
- Training completion is a governance control, not just HR recordkeeping.
...
```markdown
# Matter: [Client/Matter] — AI Policy Monitor
...
```text
Use `ai-policy-monitor` with command `scan-policy` on these documents. Store results in YAML and include section references.
```

```text
Use `ai-policy-monitor` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `ai-policy-monitor` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

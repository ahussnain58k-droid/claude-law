---
name: jurisdiction-risk-checker
title: Jurisdiction Risk Checker
version: 4.1.0
author: Ali Hussnain
...
> **perform the specific jurisdiction risk checker task with explicit commands, source references, YAML storage, obligations routing, and human-review gates.**
...
- `jurisdiction risk checker`
- `run jurisdiction risk checker`
- `create jurisdiction risk checker`
- `review jurisdiction risk checker`
...
| `route` | Route obligations/issues. |
| `export` | Export YAML and copy-paste output. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Open or create a jurisdiction risk checker record.
2. Capture sources and section references.
3. Run the specific review checklist.
4. Route obligations to the appropriate impact assessment or review gate.
5. Export copy-paste output and YAML storage.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Do not treat this as a generic workflow.
- Use source references for every finding.
- Route obligations that affect privacy, AI, public output, confidentiality, privilege, or citations.
...
```yaml
record:
  id: JURISDICTION-RISK-CHECKER-001
...
```markdown
# Matter: [Client/Matter] — Jurisdiction Risk Checker
...
```text
Use `jurisdiction-risk-checker` with command `list` on these documents. Store results in YAML and include section references.
```

```text
Use `jurisdiction-risk-checker` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `jurisdiction-risk-checker` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

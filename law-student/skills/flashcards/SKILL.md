...
> **create active-recall flashcards for legal rules, elements, exceptions, cases, and hypotheticals.**
...
| `create` | Create cards. |
| `add-card` | Add card. |
| `edit-card` | Revise card. |
| `tag-card` | Tag by topic. |
| `export-deck` | Export deck YAML. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Create short question-answer cards.
2. Use cloze only for elements or lists.
3. Add hypothetical cards for application.
4. Tag each card by topic/source.
5. Avoid long outline paragraphs as cards.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Recall cards should test memory; hypo cards test application.
- One card should test one idea.
- Rules must match course/jurisdiction materials.
...
```markdown
# Matter: [Client/Matter] — Flashcards
...
```text
Use `flashcards` with command `create` on these documents. Store results in YAML and include section references.
```

```text
Use `flashcards` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `flashcards` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

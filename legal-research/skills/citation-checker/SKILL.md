...
> **check whether each proposition has a complete, jurisdictionally appropriate, current, and supportive citation.**
...
| `add-citation` | Add citation record. |
| `link-proposition` | Link proposition to citation. |
| `mark-unverified` | Mark citation unverified. |
| `export-citation-queue` | Export queue. |
...
2. Match each proposition to citation.
3. Check citation format, court, year, jurisdiction, pinpoint, treatment status, and proposition support.
4. Flag unverified/fake citations.
5. Do not invent replacements.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Citation existence is separate from proposition support.
- Pinpoints matter for quotes and narrow holdings.
- Negative treatment requires external verification.
...
```markdown
# Matter: [Client/Matter] — Citation Checker
...
```text
Use `citation-checker` with command `check` on these documents. Store results in YAML and include section references.
```

```text
Use `citation-checker` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `citation-checker` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

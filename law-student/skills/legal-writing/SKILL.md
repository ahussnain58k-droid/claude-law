...
> **improve legal writing for thesis, structure, authority use, rule synthesis, analysis, transitions, tone, and citation support.**
...
|---|---|
| `diagnose` | Diagnose writing problems. |
| `edit-section` | Edit one section. |
| `add-authority-note` | Flag missing authority. |
| `rewrite` | Rewrite draft. |
| `export-feedback` | Export feedback memo. |
...
2. Check thesis and roadmap.
3. Check rule synthesis and authority support.
4. Check application depth and counterarguments.
5. Rewrite only requested sections and preserve legal meaning unless marked.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Clarity cannot fix unsupported legal analysis.
- Authority must support the proposition cited.
- Overstating rule strength is a legal-writing error.
...
```yaml
writing_project:
  type: memo|brief|email|exam|note
  audience: [audience]
edits:
  -
    section: [section]
...
```markdown
# Matter: [Client/Matter] — Legal Writing
...
```text
Use `legal-writing` with command `diagnose` on these documents. Store results in YAML and include section references.
```

```text
Use `legal-writing` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `legal-writing` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

...
> **coach a legal fact pattern into IRAC with issue spotting, rule, application, counterarguments, conclusion, and rubric feedback.**
...
|---|---|
| `issue-spot` | List issues. |
| `draft-irac` | Draft IRAC answer. |
| `edit-rule` | Revise rule statement. |
| `score-answer` | Score answer with rubric. |
| `export-model-answer` | Export practice answer. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify major and minor issues.
2. State rule only when supplied or clearly course-general.
3. Apply facts element by element.
4. Include both sides where facts support ambiguity.
5. Score using issue/rule/application/conclusion structure.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Application is usually worth more than rule recitation.
- Conclusions should reflect uncertainty.
...
```markdown
# Matter: [Client/Matter] — IRAC Practice
...
```text
Use `irac-practice` with command `issue-spot` on these documents. Store results in YAML and include section references.
```

```text
Use `irac-practice` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `irac-practice` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

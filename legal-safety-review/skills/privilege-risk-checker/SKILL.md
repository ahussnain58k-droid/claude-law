---
name: privilege-risk-checker
title: Privilege Risk Checker
version: 4.1.0
author: Ali Hussnain
...
> **flag attorney-client privilege, work-product, waiver, distribution, and common-interest risks before sharing or summarizing legal materials.**
...
|---|---|
| `scan` | Scan privilege risk. |
| `set-recipients` | Set sender/recipient list. |
| `mark-privileged` | Mark material. |
| `recommend-handling` | Recommend safe handling. |
| `export-privilege-note` | Export note. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify communication purpose, sender, recipients, legal advice content, litigation/work-product context, and distribution plan.
2. Flag waiver risks.
3. Separate legal advice from business content.
4. Recommend safer handling.
5. Do not make final privilege determination.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Privilege can be waived by unnecessary third-party distribution.
- Legal and business advice may be mixed.
- Common-interest requires legal review.
...
```markdown
# Matter: [Client/Matter] — Privilege Risk Checker
...
```text
Use `privilege-risk-checker` with command `scan` on these documents. Store results in YAML and include section references.
```

```text
Use `privilege-risk-checker` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `privilege-risk-checker` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

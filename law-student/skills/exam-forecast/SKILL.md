...
> **identify likely exam themes from authorized course materials and convert them into lawful practice prompts and study priorities.**
...
|---|---|
| `forecast` | Create theme forecast. |
| `add-material` | Add course material. |
| `rank-topic` | Rank topic priority. |
| `create-prompt` | Create practice prompt. |
| `export-study-priorities` | Export priorities. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Use only authorized materials.
2. Rank topics by coverage, professor emphasis, complexity, integration potential, and weakness.
3. Create practice prompts, not actual exam leaks.
4. Mark uncertainty.
5. Route to study-plan.
...
- Past exams may be unauthorized.
- Themes should become practice, not prediction dependence.
...
```markdown
# Matter: [Client/Matter] — Exam Forecast
...
```text
Use `exam-forecast` with command `forecast` on these documents. Store results in YAML and include section references.
```

```text
Use `exam-forecast` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `exam-forecast` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

...
> **create original bar-style practice questions with one best answer, distractor analysis, rule summary, and learning feedback.**
...
|---|---|
| `create-question` | Create original question. |
| `add-distractor` | Add distractor. |
| `edit-explanation` | Revise explanation. |
| `score-response` | Score user answer. |
| `export-question-set` | Export set. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Pick one tested rule.
2. Draft original facts.
3. Ensure one best answer.
4. Explain each wrong answer.
5. Add rule summary and memory hook.
6. Do not copy licensed bar questions.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Two plausible correct answers makes a bad question.
- Distractors should test common confusions.
- Jurisdiction-specific bar rules require source.
...
```markdown
# Matter: [Client/Matter] — Bar Prep Questions
...
```text
Use `bar-prep-questions` with command `create-question` on these documents. Store results in YAML and include section references.
```

```text
Use `bar-prep-questions` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `bar-prep-questions` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

---
name: source-reliability-review
title: Source Reliability Review
version: 4.1.0
author: Ali Hussnain
...
> **evaluate whether sources are reliable enough for legal research, public content, internal analysis, or background only.**
...
|---|---|
| `review` | Review source reliability. |
| `add-source` | Add source. |
| `rank-source` | Rank source. |
| `mark-limit` | Mark use limit. |
| `export-source-table` | Export table. |
...
2. Check author, publisher, date, jurisdiction, authority level, bias, and currency.
3. Decide allowed use: binding, persuasive, background, not reliable.
4. Flag replacement needs.
5. Route citation checks.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- A reliable news source may not support a legal proposition.
- AI-generated sources need independent verification.
- Official sources are preferred for statutes/regulations.
...
sources:
  -
    id: S1
    title: [title]
    type: official|case|statute|secondary|news|blog|AI|unknown
    reliability: high|medium|low
    allowed_use: binding|persuasive|background|do_not_use
```
...
```markdown
# Matter: [Client/Matter] — Source Reliability Review
...
```text
Use `source-reliability-review` with command `review` on these documents. Store results in YAML and include section references.
```

```text
Use `source-reliability-review` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `source-reliability-review` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

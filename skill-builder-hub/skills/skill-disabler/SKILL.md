...
> **disable a skill safely while preserving audit trail, user data, dependency awareness, and re-enable conditions.**
...
| `set-reenable-conditions` | Set conditions. |
| `export-disable-note` | Export note. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Confirm skill and dependencies.
2. Identify affected workflows.
3. Record reason and owner.
4. Disable without deleting user data.
5. Set re-enable conditions.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Disabling should not erase evidence of unsafe behavior.
- Dependent workflows need replacement path.
- Disabling a safety skill can increase risk.
...
```markdown
# Matter: [Client/Matter] — Skill Disabler
...
```text
Use `skill-disabler` with command `assess-impact` on these documents. Store results in YAML and include section references.
```

```text
Use `skill-disabler` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `skill-disabler` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

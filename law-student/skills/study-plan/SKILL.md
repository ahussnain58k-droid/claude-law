...
> **create a realistic law-study schedule with active recall, practice, review cycles, writing drills, and checkpoints.**
...
| `add-block` | Add study block. |
| `edit-priority` | Change topic priority. |
| `mark-complete` | Mark task done. |
| `export-calendar` | Export schedule. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Capture exam date, available blocks, weak areas, materials, and target outcome.
2. Prioritize by exam weight and weakness.
3. Balance outline, active recall, practice, writing, and rest.
4. Add checkpoints and adjustment rules.
...
- Practice should start earlier than student expects.
...
blocks:
  -
    ...
    duration: 90m
    task: [task]
    method: outline|flashcards|practice|writing|review
    status: todo|done
```
...
```markdown
# Matter: [Client/Matter] — Study Plan
...
```text
Use `study-plan` with command `create-plan` on these documents. Store results in YAML and include section references.
```

```text
Use `study-plan` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `study-plan` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

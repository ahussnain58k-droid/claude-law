---
name: related-skills-finder
title: Related Skills Finder
version: 4.1.0
author: Ali Hussnain
...
> **recommend adjacent skills based on the current job while avoiding unsafe dependency sprawl and conflicts.**
...
|---|---|
| `find` | Find related skills. |
| `check-conflict` | Check conflicts. |
| `rank-fit` | Rank fit. |
| `export-routes` | Export route list. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Identify current job.
2. Find adjacent skills that solve next legal workflow step.
3. Check overlap and conflicts.
4. Rank by utility and risk.
5. Require installer/security review before adoption.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- More skills can create conflicting instructions.
- Related skill should solve a concrete next job.
- Avoid recommending high-permission skills when low-permission workflow suffices.
...
```markdown
# Matter: [Client/Matter] — Related Skills Finder
...
```text
Use `related-skills-finder` with command `find` on these documents. Store results in YAML and include section references.
```

```text
Use `related-skills-finder` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `related-skills-finder` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

---
name: research-memo-builder
title: Research Memo Builder
version: 4.1.0
author: Ali Hussnain
...
> **convert research notes into a legal memo with question presented, short answer, facts, discussion, contrary authority, and next research tasks.**
...
| `edit-short-answer` | Revise short answer. |
| `add-gap` | Add research gap. |
| `export-memo` | Export memo. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Define question presented and facts assumed.
2. Organize authorities by issue and hierarchy.
3. Draft short answer with confidence level.
4. Write discussion with rule synthesis and application.
5. List contrary authority and gaps.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Memo conclusion depends on assumed facts.
- Contrary authority should be disclosed.
- Unverified citations must remain flagged.
...
```markdown
# Matter: [Client/Matter] — Research Memo Builder
...
```text
Use `research-memo-builder` with command `build` on these documents. Store results in YAML and include section references.
```

```text
Use `research-memo-builder` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `research-memo-builder` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

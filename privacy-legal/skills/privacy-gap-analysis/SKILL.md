...
> **map privacy requirements to existing controls, evidence, owners, gaps, remediation actions, and verification status.**
...
|---|---|
| `map-requirements` | Convert target requirement into control map. |
| `add-control` | Add current control/evidence. |
| `edit-gap` | Edit gap status/severity. |
| `assign-owner` | Assign remediation owner. |
| `route-impact` | Route high-risk processing to PIA/DPIA or DPA. |
| `export-remediation` | Export remediation plan. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Break target requirement into testable controls.
2. Map current policy, procedure, technical control, evidence, owner, and status for each control.
3. Classify gap type: documentation, process, technical, vendor, training, rights, incident, retention, transfer, governance.
4. Route missing processing assessment to privacy-impact-assessment and vendor gaps to DPA review.
5. Rank remediation by legal risk, customer/regulator impact, effort, and dependency.
...
- A control without owner and test evidence should remain incomplete.
- Privacy gaps can be legal, operational, technical, or governance gaps.
- Customer commitments may require stricter controls than baseline law.
...
```markdown
# Matter: [Client/Matter] — Privacy Gap Analysis
...
```text
Use `privacy-gap-analysis` with command `map-requirements` on these documents. Store results in YAML and include section references.
```

```text
Use `privacy-gap-analysis` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `privacy-gap-analysis` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```

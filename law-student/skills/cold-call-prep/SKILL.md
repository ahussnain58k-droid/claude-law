...
> **generate likely cold-call questions and concise answers from one assigned case or reading.**
...
|---|---|
| `prepare` | Create cold-call prep. |
| `add-question` | Add professor-style question. |
| `edit-answer` | Revise answer. |
| `drill` | Run question-by-question practice. |
| `export-one-minute-summary` | Export concise case summary. |

## 4. Executable Workflow

Follow these concrete steps for this skill:

1. Start with one-minute case summary.
2. Generate fact, posture, issue, rule, reasoning, policy, dissent, and hypothetical questions.
3. Keep answers short.
4. Add traps and follow-ups.
5. Do not give unsupported case facts.

## 5. Legal Nuance Rules

Apply these legal/domain-specific rules:

- Cold-call answers should be concise, not essay-length.
- Policy and hypotheticals often matter as much as holding.
- Student should verify against professor materials.
...
questions:
  -
    type: fact|posture|issue|rule|reasoning|policy|hypo
    question: [q]
    answer: [a]
```
...
```markdown
# Matter: [Client/Matter] — Cold Call Prep
...
```text
Use `cold-call-prep` with command `prepare` on these documents. Store results in YAML and include section references.
```

```text
Use `cold-call-prep` command `list` if available, then `edit` the YAML state after I confirm corrections.
```

```text
Use `cold-call-prep` and route any privacy, AI, privilege, citation, or public-output obligations to the correct follow-up skill.
```
